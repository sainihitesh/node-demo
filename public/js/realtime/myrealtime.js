var data=[];
var dataIndex="";
var MyRealtimegraph = function(){
        
        this.sentdatafun =function(sentdata){
            
        data.push(sentdata.d);
...
//    var voltage=45,    
	...
	var usersDeviceDoughnutChart = new CanvasJS.Chart("users-device-doughnut-chart", {
		...
		legend: {
			...
			itemTextFormatter: function (e) {
				return e.dataPoint.name + ": " + Math.round(e.dataPoint.y / voltage * 100) + "%";  
			} 
		},
		title: {
		...
		toolTip: {
			...
			contentFormatter: function (e) {
				return e.entries[0].dataPoint.name + ": " + Math.round(e.entries[0].dataPoint.y / voltage * 100) + "% (" + e.entries[0].dataPoint.y  + ")";
			} 
		},
		data: [
			{
	...
	var usersStateBarChart = new CanvasJS.Chart("users-state-bar-chart", {
		...
		toolTip: {
			...
			fontStyle: "normal",
			contentFormatter: function (e) {
				return e.entries[0].dataPoint.label + ": " + Math.round(e.entries[0].dataPoint.y / voltage * 100) + "% (" + e.entries[0].dataPoint.y  + ")";
			} 
		},
		data: [
			{
				...
				indexLabelFormatter: function (e) {
					return Math.round(e.dataPoint.y / voltage * 100) + "%";  
				},
				type: "bar",
				dataPoints: [
					{ y: 16,  label: "Others" },
	...
	function updateUsersDeviceChart(dataIndex) {
		usersDeviceDoughnutChart.options.title.text = voltage.toString();
		
        if(dataIndex >0)
            {
                
               	for (var i = 0; i < data[dataIndex].length; i++)
			usersDeviceDoughnutChart.options.data[0].dataPoints[i].y = data[dataIndex].vol; 
            }
...
console.log("update charts");	
	...
	function updateChartsAtRandomIntervals() {
       ...
       var dataIndex =data.length-1;
		...
		console.log("random interval"+updateChartsInterval);		
		updateCharts(dataIndex);
	...
	(function init() {
		...
		setTimeout(updateChartsAtRandomIntervals, 1000);
